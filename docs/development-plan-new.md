...
2. [当前进度评估（v0.13.0）](#2-当前进度评估v0130)
3. [竞品深度研究](#3-竞品深度研究)
...
| **即插即用** | `pip install` / REST API 20 端点 / Channel Adapter SDK / Docker 一键 | 规避 Hermes 60+ 参数与 OpenClaw TS 编译链 |
...
| F3 | 26 内置工具 + 3 级 Profile（minimal/standard/full） | `tools/builtin.py` | ✅ |
...
| 70+ 内置工具 + 28 toolsets | 最丰富的工具生态 | ⚠️ 26 工具，差距大 |
...
| 消息队列 4 模式 | steer/followup/collect/interrupt | ✅ v0.13.0 已实现 |
...
| 3 平台渠道适配器 | Telegram + Discord + Slack 可用 | 2 周 |
...
│  │ (20端点) │ │ (实时推送)│ │ Bot API  │ │ Bot API  │ │ Bolt SDK │  │
│  └────┬─────┘ └────┬─────┘ └────┬─────┘ └────┬─────┘ └────┬─────┘  │
│       └────────────┴────────────┴────────────┴────────────┘         │
│                                │                                     │
│                    ┌───────────▼───────────┐                         │
...
│                       Gateway 层（v0.13.0 ✅）                       │
│                                                                      │
│  FastAPI · GatewayState 类 · 20 REST 端点 · Cron 调度 · Docker 沙箱 │
└────────────────────────────────┬────────────────────────────────────┘
                                   │
┌─────────────────────────────────▼───────────────────────────────────┐
│                         Agent 核心层                                 │
...
| 演进项 | 当前（v0.13.0） | 目标（v1.0.0） |
|:---|:---|:---|
| 异步模型 | 部分（chat_async + 并行工具，BackgroundReview 仍用 thread） | 全链路 asyncio，零 threading.Thread |
...
| **M3.6e** | **Telegram 适配器** — Bot API，最易上手的渠道 | 🟡 P1 | 2d |
| **M3.6f** | **Discord 适配器** — Bot API + Gateway 集成 | 🟡 P1 | 2d |
| **M3.6g** | **Slack 适配器** — Bolt SDK | 🟡 P1 | 2d |
| **M3.8** | **Profile 隔离（F19）** — 独立 config/memory/sessions per profile，并发运行 | 🟡 P1 | 2d |

#### 7.2.4 Phase 3 收尾（v0.16.0）
...
| **M3.6h** | **飞书适配器** — WebSocket 事件订阅 | 🟢 P2 | 2d |
| **M3.6i** | **WebChat 适配器** — 基于 WebSocket 的 Web 聊天 | 🟢 P2 | 2d |
| **M3.6j** | **渠道配置热加载** — YAML 变更自动检测，无需重启 | 🟢 P2 | 1d |
| **M3.6k** | **媒体处理** — 图片/文件/语音消息处理 | 🟢 P2 | 2d |
...
| 指标 | v0.13.0 | v0.14.0 目标 | v0.15.0 目标 | v0.16.0 目标 | v1.0.0 目标 |
|:---|:---:|:---:|:---:|:---:|:---:|
...
| 差异化点 | v0.13.0 | Phase 3 目标 | v1.0.0 目标 |
|:---|:---|:---|:---|
...
| **Telegram 适配器** | python-telegram-bot，最易上手 | P1 |
| **Discord 适配器** | discord.py，Gateway 集成 | P1 |
| **Slack 适配器** | slack-bolt，Socket Mode | P1 |
...
| 任务 | 说明 | 优先级 |
|:---|:---|:---:|
| 飞书适配器 | lark-oapi，WebSocket 事件订阅 | P2 |
